...
<div x-data="jadwalManager()" class="space-y-6">
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-8 border border-slate-100 dark:border-zinc-800 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
            ...
            <div class="flex flex-wrap items-center gap-3 mt-2.5">
                <span class="inline-flex items-center gap-2 text-xs font-black bg-indigo-50 dark:bg-indigo-950/50 text-indigo-650 dark:text-indigo-400 px-4 py-2 rounded-xl border border-indigo-100 dark:border-indigo-900/30 uppercase tracking-wider shadow-sm">
                    <span class="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span>
                    {{ now()->locale('id')->isoFormat('dddd, D MMMM YYYY') }}
                </span>
                <p class="text-slate-450 dark:text-slate-500 text-sm font-medium">· Kelola jadwal bimbingan belajar guru dan rombel secara efisien.</p>
            </div>
        </div>
        <div class="flex flex-wrap items-center gap-3 shrink-0">
            {{-- Toggle Mode Tampilan Premium --}}
            <div class="flex items-center bg-slate-100 dark:bg-zinc-955 p-1 rounded-2xl border border-slate-200/50 dark:border-zinc-800/50 mr-2 shadow-inner">
            ...
            <button @click="showDuplikasi = true"
                class="inline-flex items-center gap-3 bg-white dark:bg-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-700 text-slate-700 dark:text-slate-200 font-black text-xs px-6 py-4 rounded-2xl shadow-sm border border-slate-200 dark:border-zinc-750 transition-all active:scale-95">
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-8 border border-slate-100 dark:border-zinc-800 shadow-sm relative overflow-hidden group">
        ...
        <form method="GET" action="{{ route('admin.jadwal.index') }}" class="flex flex-col lg:flex-row items-end gap-6 relative">
            <div class="flex-1 w-full space-y-3">
                ...
                <div class="relative">
                    <select name="guru_id" class="w-full bg-slate-50 dark:bg-zinc-950 border-slate-200 dark:border-zinc-800 rounded-2xl text-sm font-bold py-4 px-5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all appearance-none text-slate-850 dark:text-white">
            ...
            <div class="flex-1 w-full space-y-3">
                ...
                <div class="relative">
                    <select name="rombel_id" class="w-full bg-slate-50 dark:bg-zinc-950 border-slate-200 dark:border-zinc-800 rounded-2xl text-sm font-bold py-4 px-5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all appearance-none text-slate-850 dark:text-white">
    ...
    <div x-show="viewMode === 'calendar'" class="bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-slate-100 dark:border-zinc-800 shadow-sm overflow-hidden transition-all">
        <div class="overflow-x-auto custom-scrollbar">
            <table class="w-full min-w-[1200px] text-sm table-fixed border-collapse border border-slate-150 dark:border-zinc-800">
                ...
                <tbody class="bg-white dark:bg-zinc-900">
                    ...
                    <tr class="hover:bg-slate-50/20 dark:hover:bg-zinc-850/10 transition-colors">
                        ...
                        {{-- Sel Hari --}}
                        @foreach($hariList as $hari)
                        <td class="p-2.5 align-top min-h-[110px] h-32 {{ (now()->locale('id')->isoFormat('dddd') === $hari && $hari !== 'Minggu') ? 'bg-indigo-50/5 dark:bg-indigo-955/5' : '' }} relative border border-slate-150 dark:border-zinc-800 group">
                            @php
                            ...
                            <div class="space-y-2">
                                ...
                                <div @click="openEditModal({{ $j->toJson() }})"
                                    class="group/card p-3 rounded-2xl cursor-pointer transition-all hover:scale-[1.03] hover:shadow-lg active:scale-95 ring-1 ring-black/5 dark:ring-white/5 border-l-[5px]
                                        {{ ['border-indigo-500 bg-indigo-50/40 hover:bg-indigo-50/80 dark:bg-indigo-950/20 dark:hover:bg-indigo-950/40',
                                            'border-emerald-500 bg-emerald-50/40 hover:bg-emerald-50/80 dark:bg-emerald-950/20 dark:hover:bg-emerald-950/40',
                                            'border-amber-500 bg-amber-50/40 hover:bg-amber-50/80 dark:bg-amber-950/20 dark:hover:bg-amber-950/40',
                                            'border-rose-500 bg-rose-50/40 hover:bg-rose-50/80 dark:bg-rose-950/20 dark:hover:bg-rose-950/40',
                                            'border-purple-500 bg-purple-50/40 hover:bg-purple-50/80 dark:bg-purple-950/20 dark:hover:bg-purple-950/40',
                                            'border-cyan-500 bg-cyan-50/40 hover:bg-cyan-50/80 dark:bg-cyan-950/20 dark:hover:bg-cyan-950/40',
                                           ][$j->id % 6] }}">
                                    
                                    {{-- Subject Name --}}
                                    <div class="flex items-start justify-between gap-1.5">
                                        <p class="font-extrabold text-[11px] text-slate-900 dark:text-white leading-tight break-words group-hover/card:text-indigo-650 dark:group-hover/card:text-indigo-400 transition-colors">
                                            {{ $j->mataPelajaran?->nama ?? 'Sesi Belajar' }}
                                        </p>
                                    </div>
                                    
                                    {{-- Time Details --}}
                                    <div class="flex items-center gap-1 mt-1 text-[9px] font-black text-slate-450 dark:text-slate-400 uppercase tracking-tighter">
    ...
    <div x-show="viewMode === 'list'" x-cloak class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all">
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-6 border border-slate-100 dark:border-zinc-800 shadow-sm flex flex-col min-h-[300px]">
            ...
            <div class="flex-1 space-y-3 overflow-y-auto max-h-[350px] pr-1 scrollbar-thin">
                ...
                <div @click="openEditModal({{ $j->toJson() }})"
                    class="group p-4 bg-slate-50 hover:bg-indigo-50/30 dark:bg-zinc-950 dark:hover:bg-zinc-850 rounded-2xl border border-slate-100 dark:border-zinc-850/80 cursor-pointer transition-all active:scale-[0.98] hover:shadow-md">
                    ...
                    <div class="mt-3 pt-3 border-t border-slate-200/40 dark:border-zinc-800/40 grid grid-cols-2 gap-2 text-[10px] font-bold text-slate-500 dark:text-slate-400">
                        <div class="truncate flex items-center gap-1.5">
                            <div class="w-4 h-4 rounded bg-slate-250 dark:bg-zinc-800 flex items-center justify-center text-[8px] font-black uppercase text-slate-500 shrink-0">G</div>
                            <span class="truncate" title="{{ $j->guru?->name }}">{{ $j->guru?->name ?? '-' }}</span>
                        </div>
                        <div class="truncate flex items-center gap-1.5">
                            <div class="w-4 h-4 rounded bg-slate-250 dark:bg-zinc-800 flex items-center justify-center text-[8px] font-black uppercase text-slate-500 shrink-0">R</div>
                            <span class="truncate" title="{{ $j->rombel?->nama_kelompok }}">{{ $j->rombel?->nama_kelompok ?? '-' }}</span>
                        </div>
                    </div>

                    @if($j->ruangan)
                    <div class="mt-2.5 text-[9px] font-black text-indigo-650 dark:text-indigo-400 flex items-center gap-1 uppercase tracking-wider">
                ...
                <div class="flex-1 flex flex-col items-center justify-center text-center p-8 bg-slate-50/50 dark:bg-zinc-950/20 rounded-[2rem] border border-dashed border-slate-200 dark:border-zinc-800/80 my-auto">
                    ...
                    <p class="text-xs font-black text-slate-400 dark:text-zinc-650">Tidak Ada Jadwal Kelas</p>
                </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="absolute inset-0 overflow-hidden">
            ...
            <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10">
                <div x-show="showModal"
                    ...
                    <!-- Header -->
                    <div class="px-8 py-6 bg-slate-50 dark:bg-zinc-950 border-b border-slate-150 dark:border-zinc-850 flex items-center justify-between">
                        <div>
                    ...
                    <form :action="editId ? '{{ url('admin/jadwal') }}/' + editId : '{{ route('admin.jadwal.store') }}'" 
                        ...
                        <div class="flex-1 overflow-y-auto px-8 py-8 space-y-6 custom-scrollbar">
                            ...
                            <div class="bg-indigo-50/50 dark:bg-indigo-950/15 border border-indigo-100 dark:border-indigo-900/30 rounded-[2rem] p-5 flex gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-[11px] text-slate-450 dark:text-slate-400 mt-1 font-bold leading-normal">Sistem akan secara otomatis memeriksa dan memberikan peringatan jika ada jadwal mengajar guru yang bentrok/tumpang tindih.</p>
                                </div>
                            </div>
                            ...
                            <div class="space-y-5">
                                ...
                                <div class="bg-slate-50 dark:bg-zinc-950/40 p-6 rounded-[2rem] border border-slate-100 dark:border-zinc-800 space-y-4">
                                    <div class="space-y-2">
                                        <label class="block text-[10px] font-black text-slate-450 dark:text-slate-450 uppercase tracking-widest ml-1">Tenaga Pengajar <span class="text-rose-500">*</span></label>
                                        <div class="relative">
                                            <select name="guru_id" x-model="form.guru_id" required class="w-full bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 rounded-2xl text-sm font-bold py-4 px-5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-850 dark:text-white appearance-none">
                                    ...
                                    <div class="space-y-2">
                                        <label class="block text-[10px] font-black text-slate-450 dark:text-slate-450 uppercase tracking-widest ml-1">Hari Pelaksanaan <span class="text-rose-500">*</span></label>
                                        <div class="relative">
                                            <select name="hari" x-model="form.hari" required class="w-full bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 rounded-2xl text-sm font-bold py-4 px-5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-850 dark:text-white appearance-none">
                                ...
                                {{-- Section: Detail Kelas & Mata Pelajaran --}}
                                <div class="bg-slate-50 dark:bg-zinc-950/40 p-6 rounded-[2rem] border border-slate-100 dark:border-zinc-800 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div class="space-y-2">
                                        <label class="block text-[10px] font-black text-slate-450 dark:text-slate-450 uppercase tracking-widest ml-1">Kelompok Belajar</label>
                                        <div class="relative">
                                            <select name="rombel_id" x-model="form.rombel_id" class="w-full bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 rounded-2xl text-sm font-bold py-4 px-5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-850 dark:text-white appearance-none">
                                    ...
                                    <div class="space-y-2">
                                        <label class="block text-[10px] font-black text-slate-450 dark:text-slate-450 uppercase tracking-widest ml-1">Mata Pelajaran</label>
                                        <div class="relative">
                                            <select name="mata_pelajaran_id" x-model="form.mata_pelajaran_id" class="w-full bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 rounded-2xl text-sm font-bold py-4 px-5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-850 dark:text-white appearance-none">
                                                <option value="">— Opsional —</option>
                                ...
                                <div class="bg-slate-50 dark:bg-zinc-950/40 p-6 rounded-[2rem] border border-slate-100 dark:border-zinc-800 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div class="space-y-2">
                                        <label class="block text-[10px] font-black text-slate-450 dark:text-slate-450 uppercase tracking-widest ml-1">Jam Mulai <span class="text-rose-500">*</span></label>
                                        <input type="time" name="jam_mulai" x-model="form.jam_mulai" required class="w-full bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 rounded-2xl text-sm font-bold py-4 px-5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-850 dark:text-white">
                                    </div>

                                    <div class="space-y-2">
                                        <label class="block text-[10px] font-black text-slate-450 dark:text-slate-450 uppercase tracking-widest ml-1">Jam Selesai <span class="text-rose-500">*</span></label>
                                        <input type="time" name="jam_selesai" x-model="form.jam_selesai" required class="w-full bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 rounded-2xl text-sm font-bold py-4 px-5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-850 dark:text-white">
                                    </div>

                                    <div class="sm:col-span-2 space-y-2">
                                        <label class="block text-[10px] font-black text-slate-450 dark:text-slate-450 uppercase tracking-widest ml-1">Ruangan / Tempat</label>
                                        <input type="text" name="ruangan" x-model="form.ruangan" placeholder="Contoh: Lab Komputer, Ruang Kelas A" class="w-full bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 rounded-2xl text-sm font-bold py-4 px-5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-850 dark:text-white">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Sticky Footer -->
                        <div class="px-8 py-6 bg-slate-50 dark:bg-zinc-950 border-t border-slate-150 dark:border-zinc-850 flex items-center justify-between shrink-0">
                            ...
                            <div class="flex gap-4">
                                ...
                                <button type="submit" class="bg-indigo-650 hover:bg-indigo-700 text-white text-[10px] font-black uppercase tracking-widest px-8 py-4 rounded-2xl shadow-xl shadow-indigo-500/20 transition-all active:scale-95">Simpan Jadwal</button>
                            </div>
                        </div>
                    </form>
            </div>
            ...
            <div class="p-10">
                ...
                <form action="{{ route('admin.jadwal.duplikasi') }}" method="POST" class="mt-8 space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="target_periode_id" required class="w-full bg-slate-50 dark:bg-zinc-950 border-slate-200 dark:border-zinc-800 rounded-2xl text-sm font-bold py-4 px-5 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all appearance-none text-slate-850 dark:text-white">
