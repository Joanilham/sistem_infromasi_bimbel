...
.scanner-frame::before { top: 0; left: 0; border-top: 3px solid #10b981; border-left: 3px solid #10b981; border-radius: 4px 0 0 0; }
.scanner-frame::after  { top: 0; right: 0; border-top: 3px solid #10b981; border-right: 3px solid #10b981; border-radius: 0 4px 0 0; }
.scanner-corner-bl { position: absolute; bottom: 0; left: 0; width: 36px; height: 36px; border-bottom: 3px solid #10b981; border-left: 3px solid #10b981; border-radius: 0 0 0 4px; }
.scanner-corner-br { position: absolute; bottom: 0; right: 0; width: 36px; height: 36px; border-bottom: 3px solid #10b981; border-right: 3px solid #10b981; border-radius: 0 0 4px 0; }
.scanner-line { position: absolute; top: 10%; left: 5%; right: 5%; height: 2px; background: linear-gradient(90deg, transparent, #10b981, transparent); animation: scan 2s ease-in-out infinite; }
...
.ab-input:focus { border-color: #10b981; box-shadow: 0 0 0 3px rgba(16,185,129,0.12); }
.ab-btn-submit { background: #10b981; color: #fff; border: none; border-radius: 10px; padding: 0.7rem 1.25rem; font-size: 0.875rem; font-weight: 600; cursor: pointer; white-space: nowrap; transition: background 0.15s; }
.ab-btn-submit:hover { background: #059669; }
...
.log-item.ok  { background: #f0fdf4; border: 1px solid #bbf7d0; }
.log-item.err { background: #fff1f2; border: 1px solid #fecdd3; }
.log-item.dup { background: #fffbeb; border: 1px solid #fde68a; }
...
.log-item.ok  .log-icon { background: #dcfce7; color: #15803d; }
.log-item.err .log-icon { background: #ffe4e6; color: #be185d; }
.log-item.dup .log-icon { background: #fef3c7; color: #b45309; }
...
.dark .ab-input:focus { border-color: #10b981; }
...
.dark .log-item.ok { background: rgba(16, 185, 129, 0.1); border-color: rgba(16, 185, 129, 0.2); }
.dark .log-item.err { background: rgba(239, 68, 68, 0.1); border-color: rgba(239, 68, 68, 0.2); }
.dark .log-item.dup { background: rgba(245, 158, 11, 0.1); border-color: rgba(245, 158, 11, 0.2); }
...
<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1.5rem; flex-wrap:wrap; gap:0.75rem;">
    <div>
        <div style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.25rem;">
            ...
            <span style="font-size:0.75rem; font-weight:600; color:#10b981; text-transform:uppercase; letter-spacing:0.06em;">Absensi Masuk</span>
        </div>
        <h1 style="font-size:1.5rem; font-weight:700; color:var(--text-primary); margin:0;">Scan Absen Masuk</h1>
        <p style="font-size:0.875rem; color:var(--text-muted); margin:0.25rem 0 0;">{{ now()->translatedFormat('l, d F Y') }}</p>
    </div>
    <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
        <a href="{{ route('absensi.scan.pulang.page') }}" class="btn-secondary">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7"/></svg>
            Absen Pulang
        </a>
    ...
    <div>
        <div class="ab-card">
            <div class="ab-card-head">
                <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#10b981" stroke-width="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><path stroke-linecap="round" d="M14 14h.01M18 14h.01M14 18h.01M18 18h.01M14 22h.01"/></svg>
    ...
    <div style="display:flex; flex-direction:column; gap:1rem;">
        ...
        <div class="ab-card">
            <div class="ab-card-body" style="padding:1.25rem;">
                ...
                <div class="stat-bar">
                    <div class="stat-item">
                        <div class="stat-num" id="stat-masuk" style="color:#10b981;">0</div>
                        <div class="stat-lbl">Masuk</div>
                    </div>
                    <div class="stat-item">
                        <div class="stat-num" id="stat-gagal" style="color:#ef4444;">0</div>
                        <div class="stat-lbl">Tidak Ditemukan</div>
                    </div>
                    <div class="stat-item">
        ...
        <div class="ab-card" style="flex:1;">
            <div class="ab-card-head">
                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#6366f1" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
...
const URL_API = '{{ url("/absensi/scan-masuk") }}';

let stream     = null;
let scanLoop   = null;   // setInterval ID (BarcodeDetector) atau rAF ID (jsQR)
...
// ── Statistik ─────────────────────────────────────────────────
let stats = { masuk: 0, gagal: 0, scan: 0 };

function updateStats(type) {
    stats.scan++;
    if (type === 'ok' || type === 'dup') stats.masuk++;
    else stats.gagal++;
...
async function startCamera() {
    ...
    btn.className = 'btn-scan-toggle btn-close';
...
function startScanLoop() {
    ...
    if (barcodeDetector) {
        ...
        // Hindari promise menumpuk saat detect() belum selesai
        useInterval = true;
        let detecting = false;
        scanLoop = setInterval(async () => {
            ...
            detecting = false;
        }, 250); // 4 kali per detik — cukup cepat & tidak membebani CPU
        return;
    }

    // ── jsQR fallback: canvas diperkecil ke 480×360 ─────────────────
    // Full HD (1280×720) = ~3.7 juta piksel → jsQR lambat
    // 480×360 = ~0.5 juta piksel → 7× lebih cepat
...
function onDetect(value) {
    ...
    processing = true;
    kirim(value.trim());
    // 1200ms cooldown — cukup untuk hindari scan duplikat tapi tidak terasa lambat
...
function addLog(type, nama, jam, msg) {
    ...
    const subs  = { ok: 'Absen masuk tercatat', err: msg, dup: 'Sudah absen masuk' };
...
function clearLog() {
    ...
    stats = { masuk: 0, gagal: 0, scan: 0 };
    ['stat-masuk','stat-gagal','stat-scan'].forEach(id => document.getElementById(id).textContent = '0');
}
