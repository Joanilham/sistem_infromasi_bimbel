...
@section('title', 'Edit Kelompok Belajar')
...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-8 border border-slate-100 dark:border-zinc-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Edit Kelompok</h1>
            <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm font-medium">Ubah data kelompok belajar yang sudah ada.</p>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-slate-100 dark:border-zinc-800 shadow-sm overflow-hidden max-w-4xl mx-auto">
        ...
        <form action="{{ route('kelompok-belajar.update', $kelompokBelajar->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="p-8 space-y-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="nama_kelompok" required value="{{ old('nama_kelompok', $kelompokBelajar->nama_kelompok) }}" placeholder="Misal: Kelas 1, UTBK Intensif, dll" 
            ...
            <div class="p-8 border-t border-slate-100 dark:border-zinc-800 bg-slate-50/50 dark:bg-zinc-900/50 flex items-center justify-end gap-4">
                <a href="{{ route('kelompok-belajar.index') }}" class="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-white bg-rose-500 hover:bg-rose-600 rounded-2xl transition-all shadow-xl shadow-rose-500/20 active:scale-95">Batal</a>
