...
<div class="space-y-8 pb-12">
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-[2rem] p-6 sm:p-8 border border-slate-100 dark:border-zinc-800 shadow-sm">
        ...
        <form action="{{ route('keuangan.pemasukan.kategori.store') }}" method="POST" class="flex flex-col sm:flex-row gap-4 items-center">
            @csrf
            <div class="flex-1 relative group w-full">
                <input type="text" name="nama" value="{{ old('nama') }}" required autofocus
                    ...
                    class="w-full bg-slate-50 dark:bg-zinc-950 border border-slate-100 dark:border-zinc-800 rounded-xl text-sm py-3 px-4 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-colors">
            </div>
            <button type="submit" 
                class="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-black px-6 py-3 rounded-xl text-sm transition-colors shadow-sm shadow-indigo-500/30 flex items-center justify-center gap-2 shrink-0">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                        ...
                        <tr class="hover:bg-indigo-50/50 dark:hover:bg-indigo-900/10 transition-all even:bg-slate-50/30 dark:even:bg-zinc-800/30 group">
                            ...
                            <td class="px-6 py-5">
                                <div class="flex items-center justify-center">
                                    <form action="{{ route('keuangan.pemasukan.kategori.destroy', $k->id) }}" method="POST" onsubmit="event.preventDefault(); confirmDelete('Hapus Kategori?', 'Kategori yang dihapus tidak dapat dikembalikan!', this)">
                                        @csrf @method('DELETE')
                                        <button type="submit" 
