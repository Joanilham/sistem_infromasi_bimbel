    ...
    <div class="mb-4">
        ...
        <nav class="space-y-1">
            ...
            <a href="{{ route('admin.pendaftaran.index') }}"
                class="flex items-center px-3 py-1.5 rounded-2xl transition-all duration-200 {{ request()->routeIs('admin.pendaftaran.*') ? 'bg-cyan-500 shadow-md shadow-cyan-500/30' : 'hover:bg-slate-100 dark:hover:bg-zinc-800' }} group">
                <div class="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mr-3 {{ request()->routeIs('admin.pendaftaran.*') ? 'bg-white/25' : 'bg-cyan-500 shadow-sm shadow-cyan-500/40' }}">
            ...
            <div x-data="{ open: {{ request()->routeIs('peserta-didik.*') ? 'true' : 'false' }} }">
                <button @click="open = !open"
                    class="w-full flex items-center justify-between px-3 py-1.5 rounded-2xl transition-all duration-200 {{ request()->routeIs('peserta-didik.*') ? 'bg-emerald-500 shadow-md shadow-emerald-500/30' : 'hover:bg-slate-100 dark:hover:bg-zinc-800' }} group focus:outline-none">
                    <div class="flex items-center">
                        <div class="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mr-3 {{ request()->routeIs('peserta-didik.*') ? 'bg-white/25' : 'bg-emerald-500 shadow-sm shadow-emerald-500/40' }}">
                            <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="mt-1 ml-3 pl-3 border-l-2 border-slate-100 dark:border-zinc-700 space-y-1" style="{{ request()->routeIs('peserta-didik.*') ? '' : 'display: none;' }}">
                    <a href="{{ route('peserta-didik.index') }}"
                        ...
                        <div class="w-6 h-6 rounded-lg flex items-center justify-center {{ request()->routeIs('peserta-didik.index') ? 'bg-emerald-500' : 'bg-emerald-100 dark:bg-emerald-900/50' }}">
                    ...
                    <a href="{{ route('peserta-didik.keluar') }}"
                        ...
                        <div class="w-6 h-6 rounded-lg flex items-center justify-center {{ request()->routeIs('peserta-didik.keluar') ? 'bg-rose-500' : 'bg-rose-100 dark:bg-rose-900/50' }}">
                    ...
                    <a href="{{ route('peserta-didik.lulus') }}"
                        ...
                        <div class="w-6 h-6 rounded-lg flex items-center justify-center {{ request()->routeIs('peserta-didik.lulus') ? 'bg-indigo-500' : 'bg-indigo-100 dark:bg-indigo-900/50' }}">
            ...
            <div x-data="{ open: {{ request()->routeIs('manajemen-guru.*') ? 'true' : 'false' }} }">
                <button @click="open = !open"
                    class="w-full flex items-center justify-between px-3 py-1.5 rounded-2xl transition-all duration-200 {{ request()->routeIs('manajemen-guru.*') ? 'bg-amber-500 shadow-md shadow-amber-500/30' : 'hover:bg-slate-100 dark:hover:bg-zinc-800' }} group focus:outline-none">
                    <div class="flex items-center">
                        <div class="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mr-3 {{ request()->routeIs('manajemen-guru.*') ? 'bg-white/25' : 'bg-amber-500 shadow-sm shadow-amber-500/40' }}">
                            <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" />
                            </svg>
                        </div>
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="mt-1 ml-3 pl-3 border-l-2 border-slate-100 dark:border-zinc-700 space-y-1" style="{{ request()->routeIs('manajemen-guru.*') ? '' : 'display: none;' }}">
                    <a href="{{ route('manajemen-guru.index') }}"
                        ...
                        <div class="w-6 h-6 rounded-lg flex items-center justify-center {{ request()->routeIs('manajemen-guru.index') ? 'bg-amber-500' : 'bg-amber-100 dark:bg-amber-900/50' }}">
                    ...
                    <a href="{{ route('manajemen-guru.keluar') }}"
                        ...
                        <div class="w-6 h-6 rounded-lg flex items-center justify-center {{ request()->routeIs('manajemen-guru.keluar') ? 'bg-rose-500' : 'bg-rose-100 dark:bg-rose-900/50' }}">
