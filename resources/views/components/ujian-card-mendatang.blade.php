...
<div class="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow relative">
    ...
    <div class="p-5 pl-7 opacity-80 hover:opacity-100 transition-opacity">
        <div class="flex items-start justify-between gap-3 mb-3">
            <div class="flex-1 min-w-0">
                ...
                <h3 class="font-bold text-slate-700 text-base leading-snug">{{ $ujian->judul }}</h3>
                @if($ujian->deskripsi)
        ...
        <div class="flex flex-wrap gap-4 text-sm text-slate-500 mb-5 bg-white p-3 rounded-xl border border-slate-100">
        ...
        <div class="flex items-center justify-center gap-2 p-3 bg-slate-100 rounded-xl text-slate-600 text-sm font-medium">
            ...
            <span class="text-xs font-normal text-slate-400 ml-1">({{ $ujian->waktu_mulai->format('d M, H:i') }})</span>
        </div>
    </div>
</div>
