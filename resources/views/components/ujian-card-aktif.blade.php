...
<div class="bg-emerald-50 rounded-2xl border border-emerald-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow relative">
    <div class="absolute top-0 left-0 w-1.5 h-full bg-emerald-500"></div>
    <div class="p-5 pl-7">
        <div class="flex items-start justify-between gap-3 mb-3">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1.5">
                    <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 animate-pulse flex items-center gap-1.5">
                        <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Sedang Berlangsung
                    </span>
                    <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-white border border-emerald-200 text-emerald-600">
                        {{ ucfirst($ujian->mode) }}
                    </span>
                </div>
        ...
        <div class="flex flex-wrap gap-4 text-sm text-slate-600 mb-5 bg-white bg-opacity-60 p-3 rounded-xl border border-emerald-100">
            ...
            <div class="flex items-center gap-1.5 ml-auto text-emerald-700 font-semibold" 
                 x-data="{ 
                    sisa: {{ (int)$detikSisa }},
                    formatWaktu(detik) {
