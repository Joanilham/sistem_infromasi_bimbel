        ...
        <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            ...
            <div class="flex flex-col sm:flex-row gap-3 shrink-0">
                <div class="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/30 dark:bg-black/30 backdrop-blur-md border border-white/40 dark:border-white/10 text-xs sm:text-sm font-bold shadow-sm text-white">
                    ...
                    {{ $peserta?->kelompokBelajar?->nama_kelompok ?? 'Menunggu Rombel' }}
                </div>
        ...
        <div class="relative bg-amber-50/90 border border-amber-200 shadow-sm rounded-[2rem] overflow-hidden">
            ...
            <div class="relative z-10 p-6 sm:p-8 h-full flex flex-col flex-col sm:flex-row items-center sm:items-start gap-6">
                ...
                <div class="text-center sm:text-left flex-1">
                    <h3 class="text-xl font-black text-amber-900 mb-2">Pendaftaran Diproses ⏳</h3>
                    <p class="text-sm font-medium text-amber-800/80 leading-relaxed mb-4 max-w-3xl">
                        {{ session('pending_message') ?? 'Akun Anda sedang ditinjau. Menu kelas akan otomatis terbuka setelah administrasi diselesaikan.' }}
                    </p>
                    <a href="{{ route('siswa.pembayaran.index') }}" class="inline-flex items-center gap-2 rounded-xl bg-amber-600 px-6 py-2.5 text-xs font-bold text-white shadow-lg shadow-amber-600/20 transition hover:bg-amber-700 hover:-translate-y-0.5">
                        Cek Tagihan
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3" /></svg>
                    </a>
                </div>
            </div>
        </div>
