...
<body class="bg-slate-50 text-slate-900 antialiased">
    ...
    <!-- Hero Section -->
    <section class="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden min-h-[85vh] flex items-center bg-slate-900 w-full">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center w-full">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-4 px-6" data-aos="fade-up" data-aos-delay="300">
                ...
                <a href="#tentang" class="w-full sm:w-auto bg-white/10 hover:bg-white/20 border-2 border-white/20 text-white font-bold px-10 py-4 rounded-2xl transition-all backdrop-blur-md flex items-center justify-center">Tentang Kami</a>
            </div>
        </div>
    </section>
                    ...
                    <div class="group bg-white rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 flex flex-col h-full relative" data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 100 }}">
                        ...
                        @if($paket->harga_coret && $paket->harga_coret > $paket->nominal)
                            ...
                            <div class="absolute top-5 left-5 z-20">
                                <div class="bg-red-500 text-white text-[10px] font-black px-3 py-1.5 rounded-xl shadow-lg">
                                    {{ $diskon }}% OFF
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="relative h-56 sm:h-64 overflow-hidden">
                            ...
                            @else
                                <div class="w-full h-full bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center">
                                    <svg class="w-16 h-16 text-white/20" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3L1 9l11 6 9-4.91V17h2V9L12 3z"/></svg>
                                </div>
                            @endif
                            <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-60"></div>
                            @if($paket->label_populer)
                                <div class="absolute bottom-5 left-5">
                                    <span class="bg-[#F97316] text-white text-[10px] font-black px-4 py-1.5 rounded-full shadow-lg">✨ {{ $paket->label_populer }}</span>
                                </div>
                            @endif
                        </div>

                        <div class="p-8 flex-grow flex flex-col">
                            <h3 class="text-2xl font-black text-slate-900 mb-1 group-hover:text-indigo-600 transition-colors tracking-tight line-clamp-2 break-words">{{ $paket->nama_paket }}</h3>
                            <p class="text-indigo-600 text-[10px] uppercase tracking-[0.2em] mb-6 font-black break-words">{{ $paket->target_peserta ?? 'Semua Jenjang' }}</p>
                                ...
                                @forelse($topBenefits as $benefit)
                                    <div class="flex items-start gap-3">
                                        <div class="flex-shrink-0 w-5 h-5 rounded-full bg-yellow-100 flex items-center justify-center mt-0.5">
                                            <svg class="w-3 h-3 text-yellow-600" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                        </div>
                            ...
                            <div class="mt-auto bg-slate-50 rounded-[2rem] p-5 border border-slate-100">
                                @if($paket->harga_coret)
                                    <div class="text-[10px] text-slate-400 line-through mb-0.5 font-bold">Rp {{ number_format($paket->harga_coret, 0, ',', '.') }}</div>
                                @endif
                                <div class="flex items-baseline gap-1">
                                    <span class="text-2xl font-black text-indigo-600">Rp {{ number_format($paket->nominal, 0, ',', '.') }}</span>
                                    @if($paket->durasi_jumlah)
                                        <span class="text-[10px] text-slate-500 font-black uppercase">/ {{ $paket->durasi_jumlah }} {{ $paket->durasi_satuan }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center">
                <div class="order-2 lg:order-1 relative" data-aos="fade-right">
                    <div class="bg-slate-50 p-10 sm:p-14 rounded-[3rem] border border-slate-100 relative z-10 shadow-2xl shadow-slate-100/50">
                        ...
                        <p class="text-slate-600 italic text-xl leading-relaxed mb-12 border-l-4 border-indigo-600 pl-8">
                            "Menjadi lembaga pendidikan terdepan yang mengintegrasikan teknologi modern dengan metode pembelajaran efektif."
                        </p>
                        <h3 class="text-2xl font-black text-slate-900 mb-8 flex items-center gap-4">
                            <span class="w-10 h-10 rounded-2xl bg-indigo-600 text-white flex items-center justify-center text-sm shadow-lg shadow-indigo-100">M</span>
                            Misi Institusi
                        </h3>
                        <ul class="space-y-6 text-slate-600 font-bold">
                            @foreach(['Fasilitas Pembelajaran Digital','Kurikulum Adaptif Standar Tinggi','Evaluasi Sistem CBT Akurat'] as $index => $misi)
                                <li class="flex items-center gap-5">
                                    <div class="bg-white text-indigo-600 font-black rounded-xl w-8 h-8 flex items-center justify-center flex-shrink-0 text-xs shadow-md border border-slate-100">{{ $index + 1 }}</div>
                                    <span class="text-sm uppercase tracking-wider">{{ $misi }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </div>
                ...
                <div class="order-1 lg:order-2" data-aos="fade-left">
                    ...
                    <a href="{{ route('daftar.step1') }}" class="inline-flex items-center gap-5 bg-indigo-600 text-white font-black px-10 py-5 rounded-2xl hover:bg-indigo-700 transition-all shadow-2xl shadow-indigo-200 active:scale-95 group">
                        Mulai Bergabung
                        <svg class="w-6 h-6 group-hover:translate-x-2 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                    </a>
                </div>
            </div>
        </div>
    ...
    <!-- Footer Section -->
    <footer class="bg-slate-900 pt-24 pb-12 relative overflow-hidden w-full">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <!-- Mobile Back to Top Button -->
            <button onclick="window.scrollTo({top: 0, behavior: 'smooth'})" class="md:hidden absolute -top-12 right-6 w-14 h-14 bg-indigo-600 text-white rounded-full flex items-center justify-center shadow-2xl active:scale-90 transition-all">
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path d="M5 10l7-7m0 0l7 7m-7-7v18"/></svg>
            </button>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-16 mb-24">
                <div class="col-span-1 md:col-span-2 space-y-10 text-center md:text-left">
                    <a href="#" class="flex items-center gap-4 justify-center md:justify-start">
                        ...
                        @endif
                        <span class="font-black text-3xl text-white tracking-tight">{{ $masterData->nama_lembaga ?? 'Genius Education' }}</span>
                    </a>
                    <p class="text-slate-400 text-lg leading-relaxed max-w-sm font-medium mx-auto md:mx-0">
                ...
                <div class="text-center md:text-left">
                    <h4 class="font-black text-white mb-8 uppercase tracking-[0.2em] text-xs">Program</h4>
                    <ul class="space-y-5 mb-10">
                        @foreach($pakets->take(4) as $p)
                            <li><a href="{{ route('paket.detail', $p->id) }}" class="text-slate-400 hover:text-white transition-colors text-sm font-bold flex items-center gap-3 group justify-center md:justify-start">
                ...
                <div class="text-center md:text-left">
                    <h4 class="font-black text-white mb-8 uppercase tracking-[0.2em] text-xs">Pendaftaran & Akun</h4>
            ...
            <div class="pt-10 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-6">
                <p class="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] text-center md:text-left">
                    &copy; {{ date('Y') }} {{ $masterData->nama_lembaga ?? 'Genius Education' }}. Sistem Manajemen Pendidikan Terpadu.
                </p>
                <div class="flex items-center gap-8 text-[10px] font-black text-slate-600 uppercase tracking-widest">
                    <span class="hover:text-white transition-colors cursor-help">T&C</span>
                    <span class="hover:text-white transition-colors cursor-help">Privacy Policy</span>
                </div>
            </div>
        </div>
