...
class PemasukanController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $totalCabangIni = Pemasukan::when($kantorId, fn($q) => $q->whereHas('user', fn($qu) => $qu->where('kantor_id', $kantorId)))
            ->when($startDate, fn($q) => $q->whereDate('tanggal', '>=', $startDate))
            ->when($endDate, fn($q) => $q->whereDate('tanggal', '<=', $endDate))
            ->sum('nominal');

        // Total Seluruh Cabang (nationwide aggregate)
        $totalSeluruhCabang = Pemasukan::when($startDate, fn($q) => $q->whereDate('tanggal', '>=', $startDate))
            ->when($endDate, fn($q) => $q->whereDate('tanggal', '<=', $endDate))
            ->sum('nominal');
