...
class KonteksController extends Controller
{
    ...
    public function selectContext(Request $request)
    {
        ...
        if ($isSuperAdmin) {
            ...
            $assignedKantorId = $user->kantor_id ?: (\App\Models\MasterData\Kantor::first()->id ?? null);
            if ($assignedKantorId) {
                $kantors = \App\Models\MasterData\Kantor::where('id', $assignedKantorId)->get();
            } else {
                $kantors = collect();
            }
        }

        $periodes = \App\Models\MasterData\Periode::all();
    ...
    public function update(Request $request)
    {
        ...
        $request->validate([
            'kantor_id' => ['nullable', function($attribute, $value, $fail) {
                if ($value !== 'all' && !in_array($value, \App\Models\MasterData\Kantor::pluck('id')->toArray())) {
